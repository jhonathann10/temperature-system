...
func (g *GetTemperatureUseCase) Execute(cep string) (*WeatherDTO, *httperror.HttpError) {
	localidade, err := g.viaCepInterface.GetAddressByCEP(cep)
	if err != nil {
	...
	weather, errWeatherByCity := g.weatherInterface.GetWeatherByCity(localidade.Localidade)
